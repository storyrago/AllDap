...
// 계단식 7단계. 램프를 두지 않는다. 꺾이는 지점이 단계 경계에 맞아떨어져야
// "20 은 버텼고 40 에서 깨졌다" 를 표로 읽을 수 있다. 램프를 두면 꺾임이 두 단계
// 사이로 번지고, 이 PR 의 산출물이 정확히 그 숫자다.
//
// 🔴 2026-09-12: before(스레드 40)의 여섯 칸에 <160 한 칸을 더했다.> 늘린 이유가
//    "더 세게" 가 아니다. 스레드 상한을 80 으로 올린 뒤의 천장 예측이
//    80 ÷ 1.68초 = 47.6 req/s 인데, 그 천장에 닿으려면 동시 요청이 80건은 되어야 한다.
//    즉 <옛 사다리의 마지막 칸(80 VU)이 새 벽과 정확히 겹친다.> 80 까지만 돌리면
//    처리량이 47 까지 선형으로 오르는 것만 보이고 <꺾이는지는 못 본다> =
//    "벽이 어디로 옮겨갔나" 라는 이 판의 질문에 답할 수 없다.
//
// 🔴 그리고 40 이하를 <버리지 않는다.> before 에서 그 구간이 선형이었으므로 after 에서도
//    선형이어야 "원래 느렸던 것이 아니다" 를 보일 수 있고, before/after 를 한 표에
//    올리려면 같은 칸이 필요하다. 짧게 줄이고 싶은 유혹이 드는 자리라 근거를 적어둔다.
//
// ⚠️ loadtest/s2_probe.py 의 STAGES·HOLD_S 와 <같아야> 단계별 내부 지표가 올바른
//    칸으로 접힌다. 한쪽만 고치지 말 것.
const STAGES = DRYRUN ? [1] : [1, 5, 10, 20, 40, 80, 160];
