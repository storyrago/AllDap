...
class Settings(BaseSettings):
    ...
    #
    # ── 2026-08-12 측정: 가설이 <반증됐다.> 기본값을 False 로 되돌린다 ──
    #
    #      리랭커 단독   0.844  →  +융합  0.844   (같다)
    #      둘 다         0.875  →  +융합  0.782   (나빠졌다)
    #
    ...
    #    끌어올리기도 똑같이 막는다. 제로섬이고, 이 테스트셋에서는 손해였다.
    #    → 밀어내기를 고치려면 "언제 리랭커를 믿을지"를 아는 신호가 필요하다.
    #      단순 순위 평균으로는 안 된다.
    #
    #    ⚠️ 표본이 설정당 1회뿐이다 (Cloudflare 하루 한도 소진으로 나머지 4회가 429 로 죽었다).
    #      편차 폭이 0.032 이므로 -0.093 은 유의미해 보이지만 <확정은 못 했다.>
    #      한도가 풀리면 설정당 3회로 다시 잴 것.
