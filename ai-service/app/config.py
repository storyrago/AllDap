...
class Settings(BaseSettings):
    ...
    # 💰 비용: 질문당 리랭커 호출이 1회 는다. <실측하지 않았다> —
    #    대시보드를 읽어야 정확한데 그건 사람이 해야 한다. 추정 근거만 남긴다:
    #    임베딩 실측이 텍스트 117건에 12.87 뉴런(≈0.11/건)이고, 리랭커는 회당
    #    16질문 × 후보 20개 = 320건을 보낸다 → 평가 1회에 <수십 뉴런> 규모다.
    #    평가 1회 총량이 1,190 뉴런이므로 한 자릿수 %로 보이지만 <확인된 값이 아니다.>
    #    하루 8회라는 기존 계산이 흔들릴 정도는 아닐 것으로 보되, 대시보드로 한 번 확인할 것.
    #
