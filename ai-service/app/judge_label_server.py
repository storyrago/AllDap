...
// 🔴 한 건씩 보내도 경쟁이 사라지지는 않는다. 서버의 읽기·고치기·쓰기에 잠금이 없어서,
//    거의 동시에 온 두 저장은 나중에 쓴 쪽이 이긴다. 메모를 입력하고 바로 버튼을 누르면
//    메모 저장(옛 라벨)과 라벨 저장이 함께 나가 라벨이 옛 값으로 되돌아갈 수 있다.
//    1인 · 1탭 로컬 도구라 막지 않았다. 2026-09-25 에 44건을 확인했을 때 메모와 라벨이
//    어긋난 건은 없었다(메모가 없는 건에서 되돌아갔다면 파일만으로는 알 수 없다).
let data = null, i = 0;
