...
✅ 사람 라벨과 대조했다 (2026-09-25, 44건 전수)
─────────────────────────────────────────────────────────────────────────────
run 11~16 의 채점 결과에서 중복과 fallback 을 뺀 44건 <전부>에 사람이 라벨을 매겨
대조했다(표본이 아니라 모집단이다). 결과 전문은 AGENTS.md 의
"채점자를 사람 라벨과 대조했다 (2026-09-25)" 절에 있다.

  일치 40건 (90.9%)  ·  채점자가 후함 2건  ·  박함 2건  ·  선형가중 카파 0.509
  전체충실성 왜곡량   306청크 -0.0577  /  360청크 +0.0000 ~ +0.0192

🔴 이 숫자를 "채점자를 믿어도 된다" 로 읽지 말 것. 말할 수 있는 것은 셋이다.
  ① 같은 코퍼스 위의 설정 A·B 비교는 왜곡이 양쪽에 공통으로 실려 상쇄된다.
  ② <코퍼스를 가로지르는> 비교는 위험하다. 왜곡 자체가 코퍼스마다 다르다
     (-0.0577 ↔ +0.0192). 2026-09-18 에 코퍼스를 키우고 내려간 0.0577 은
     사람 기준으로는 오히려 +0.0128 이었다. 즉 답이 나빠진 것이 아니었다.
  ③ 카파만 인용하지 말 것(카파 역설). 44건 중 40건이 한 칸에 몰려 있다.

⚠️ 한계: 한 봇 · 한 코퍼스 · 이 모델 하나이고, 최종 라벨은 재검토 단계에서
   블라인드가 아니었다(1차 블라인드 수치도 그 절에 함께 남겼다).
   그리고 사람과 채점자가 <같이> 틀린 건은 이 대조로 잡히지 않는다.
"""
