...
### 🔴 프론트는 자동 배포, 백엔드는 수동이다 — 이 비대칭이 운영을 깨뜨린다 (2026-09-08 실제로 겪음)

| | 배포 방식 |
|---|---|
| 프론트 (Vercel) | `main` 에 푸시하면 **자동** |
| 백엔드 (EC2) | **수동.** 아래 4단계를 사람이 돌려야 한다 |
...
**그래서: API 컨트랙트가 바뀐 PR 을 머지했으면 <곧바로> 아래 4단계를 돌릴 것.**
머지와 배포 사이의 시간이 곧 운영이 깨져 있는 시간이다.
