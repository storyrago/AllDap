...
- **환경변수(선택)**: `NEXT_PUBLIC_DEMO_PUBLIC_KEY` — 공개 `/demo` 화면이 대화할 봇의 publicKey.
  **미설정이면 `pk_local_dev`(시드 봇. 2026-09-13 부터 `V9__bigint_ids.sql` 이 `id = 1` 로 만든다)로 떨어진다.** 지금 운영이 그 상태이고, 그래서
...
⚠️ **설정 조회는 시드 봇 `pk_local_dev`(V9 가 `id = 1` 로 만든다) 에 딸려 있다.** 그 봇을 지우면 배포가 빨간불이 된다
...
| `bdabaa4` · `a0ac224` · `7157b9d` (#90·#89·#88 머지) | ❌ 없음 | **cancelled** (2026-09-10 에 원인을 고쳤다. 아래 ②) |
...
- 🔴 **머지 커밋인데도 없을 수 있다. 원인이 둘이고 서로 다르다.**
  ...
  ✅ **2026-09-10 에 껐다.** 그전에는 main 에 머지가 몰리면 앞선 실행이 취소돼
  <어느 SHA 로 되돌릴 수 있는가>가 그 커밋의 내용이 아니라 **그날 머지가 몰렸는지**에
  달려 있었다. 2026-09-09 07:54~07:55 에 PR 셋이 1분 안에 연달아 머지되자 앞의 둘이
  취소돼 **이미지가 영영 안 만들어졌다.**
  🔴 **고친 것은 앞으로뿐이다. 이미 빠진 이미지는 소급해서 생기지 않는다.** 위 표의
  `bdabaa4`·`a0ac224`·`7157b9d` 는 지금도 없고 앞으로도 없다. **2026-09-10 이전 구간으로
  되돌릴 때는 여전히 아래 명령으로 확인해야 한다.**
...
V7 이전 코드의 조회는 `Optional<BillingMethod> findByUserId(...)` **하나뿐**이고,
(그때 인자 타입은 `UUID` 였다. 2026-09-13 BIGINT 전환 뒤로는 `Long` 이지만 이 이야기의 요점은 타입이 아니라 <제약이 사라진 채로 남는다> 는 것이다.)
...
## 🔴 V9: 운영 DB 를 비운다 (2026-09-13 BIGINT 전환)

**되돌릴 수 없는 작업이다. 그래서 절차를 여기 적어둔다.**

`V9__bigint_ids.sql` 은 기본키를 UUID v4 에서 `BIGINT ... IDENTITY` 로 바꾸면서
**전 테이블을 DROP 하고 다시 만든다.** 즉 **운영 데이터(계정·봇·문서·청크·대화 로그·평가 결과)가
전부 사라진다.** 2026-09-13 에 알고 택한 것이다(`docs/decisions.md` 2026-09-13 항목).

### 손으로 비우지 않는다. Flyway 가 한다

🔴 **RDS 에 들어가 `DROP DATABASE` 를 하지 말 것.** V9 자체가 DROP 으로 시작하므로
**배포가 끝나면 운영 DB 는 저절로 비워진다.** 손으로 먼저 비우면 `flyway_schema_history`
까지 사라져 V1~V8 을 다시 적용하는 경로로 빠지고, 그건 이 문서 3번 항목의
`Found non-empty schema(s) "public" but no schema history table` 로 이어진다.

### 🔴 위험한 것은 "스키마만 바뀌고 앱은 옛 이미지" 인 상태다

`deploy.yml` 이 Spring 을 올리는 순간 Flyway 가 V9 를 적용한다. **그 뒤 배포가 실패하면
DB 는 BIGINT 인데 도는 앱은 UUID 를 기대하는 옛 이미지다.** 그 조합은 조용히 뜨지 않고
`ddl-auto: validate` 에서 막히지만(타입이 다르다), **막히는 것과 서비스가 도는 것은 다르다.**
그리고 이 문서 ④항이 이미 기록한 대로, **Flyway 는 "옛 앱 + 새 스키마" 를 막아주지 않는다.**

그래서 순서를 지킨다.

1. PR 을 머지한다.
2. `publish.yml` 이 성공하는 것을 본다 (이미지가 레지스트리에 올라갔다).
3. `deploy.yml` 이 백엔드를 배포한다. **여기서 V9 가 적용되고 DB 가 비워진다.**
   `scripts/verify-deploy.sh` 가 <pull 로 받은 이미지 ID>와 <컨테이너가 도는 이미지 ID>를
   대조하므로, 옛 이미지가 도는 채로 초록불이 나지 않는다.
4. Vercel 이 프론트를 배포한다. 🔴 **여기까지 안 끝났으면 종단 확인을 시작하지 말 것** -
   옛 프론트가 UUID 를 문자열로 들고 새 백엔드를 부르면 실패하는데, 그 실패는
   "전환이 틀렸다" 와 구분되지 않는다.
5. **둘 다 끝난 뒤** `https://alldap.duckdns.org` 에서 종단으로 확인한다:
   가입 → 봇 생성 → 문서 업로드(`ready` 까지) → 근거 붙은 답변.
   **기존 계정은 없어졌으므로 로그인이 아니라 <가입>부터 한다.**
6. `/demo` 가 데모 봇으로 도는지 확인한다. **그 봇은 이제 `id = 1` 이다** (V9 끝의 시드).
   `NEXT_PUBLIC_DEMO_PUBLIC_KEY` 미설정 상태라면 `pk_local_dev` 로 떨어지고, 그게 정상이다.

### 되돌리려면

**"되돌린다" 는 데이터를 되살리는 것이 아니다.** 롤백 경로는 하나뿐이다:
운영 DB 를 비우고 V8 까지로 다시 만든 뒤, 앱을 V9 이전 SHA 로 되돌린다
(백엔드 SHA + Vercel Promote 둘 다. 위 "롤백" 절 참고).
**V9 이전의 운영 데이터는 어느 쪽으로 가도 돌아오지 않는다.**

⚠️ 되살릴 값어치가 있는 데이터가 있다면 배포 <전에> 덤프를 떠둘 것.
RDS 자동 스냅샷은 남지만, **그 스냅샷은 UUID 스키마라 V9 이후 앱에 붙지 않는다.**
읽을 수는 있어도 그대로 복원해 쓸 수는 없다.
