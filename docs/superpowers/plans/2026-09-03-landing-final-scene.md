...
> ⚠️ 위 스니펫의 "0.999 로 재는 이유" 주석은 실행 당시 실제로 쓴 근거와 다르다.
> 이 루프는 지수 보간이 아니라 <시간 기반으로 정해진 길이를 재생>하는 방식이라
> `k >= 1` 이면 `p` 는 `target` 에 정확히 도달한다 — "정확히 1 이 되는 프레임을 놓칠 수
> 있다"는 성립하지 않는다. 진짜 이유는 부동소수점 오차(`transFrom + (target - transFrom)
> * eased` 계산이 반올림으로 `target` 과 1비트 어긋날 수 있다)와, 마지막 프레임까지
> 기다릴 이유가 없다는 것(저장은 한 번이면 충분)이다. 계획서는 실행 기록이라 스니펫
> 자체는 고치지 않았고, 커밋 `fde1fbc` 가 실제 코드 주석을 이 근거로 정정했다.
