...
**브라우저로 끝까지 확인했다 (2026-09-08).** 함수 반환값만 보고 "취약점" 이라 부르지
않으려고 dev 서버에서 실제로 눌러봤다. `/auth` 의 로그인 가드는 토큰이 <있기만> 하면
`router.replace(nextPath())` 를 부르므로 가짜 토큰으로 재현된다.
...
**Next 라우터가 프로토콜 상대 URL 을 해석해 외부 도메인까지 실제로 나간다.**
결함이 아니라 취약점이다. 남은 미측정은 하나뿐이다: 운영(Vercel)에서 직접 누르지는
않았다. 같은 코드가 같은 라우터로 도는 것이 근거이고, 운영에서 시도하면 실제 사용자
계정과 로그가 얽힌다.
...
🔴 **B-3·B-5 와 `PLANS.find` 뭉갬은 TDD 로 고친다 (2026-09-08 자기검토).** 처음 설계는 JSX 를 재배치하고 브라우저로 눈으로 보는 것이었는데, 그러면 **CI 가 아무것도 지키지 않는다.** 이 정리의 핵심 교훈이 *"짜둔 검사가 안 돌고 있었다"* 인데 같은 묶음에서 검사 없는 수정을 하는 것은 앞뒤가 안 맞는다.
뽑아낼 값어치가 있는 이유는 **깨진 것이 정확히 <분기 판단>이기 때문**이다. 마크업이 아니라 "어떤 상태에서 무엇을 그릴지" 가 틀렸고, 그 판단은 렌더러 없이 잴 수 있다. 저장소에 이미 같은 방식이 여섯 개 있다(`redirect.check.ts` · `retriever_check.py` · `evaluator_check.py` · `answerable_check.py` · `bot_prompt_check.py` · `fallback_e2e_check.py`).

- `web/lib/wallet.ts` + `wallet.check.ts` (18케이스, 수정 전 4건 빨간불)
- `web/lib/plans.ts` 의 `resolvePlan` + `plans.check.ts` (4케이스, 수정 전 2건 빨간불)
- 둘 다 `npm run check` 가 돌리고 CI 가 그걸 돌린다

🔴 **B-4(`planFailed`)와 접근성·metadata 에는 자동 검사를 안 붙인다.** 판단이 렌더 가드 한 줄이라 순수 함수로 뽑아도 껍데기가 되고, 실제로 재려면 렌더러가 필요한데 이 저장소에 프론트 테스트 러너가 없다. 러너 도입은 별도 슬라이스다. 대가는 그 셋이 브라우저와 눈으로만 지켜진다는 것이다.

🔴 **B-3 과 B-5 는 한 커밋으로 묶는다.** 둘 다 같은 `<details>` 줄을 고치기 때문이다. 나누면 한 PR 안에서 같은 줄을 두 번 바꾸는 커밋이 남고, 리뷰어는 첫 번째 버전을 읽을 이유가 없다.
...
| `pricing/page.tsx:43` metadata description | 숫자 3개를 `lib/plans.ts` 와 두 벌로 적고, 화면에서 뺀 "가정값" 을 계속 말한다. 🔴 처음에는 "문구 한 줄" 이라 묶음 ② 에 넣었다가 **묶음 ③ 으로 옮겼다** (2026-09-08 자기검토): 바꾸는 이유를 설명하는 데 세 줄이 들었고, **설명이 필요하면 PR 이라는 것이 이 저장소의 기준**이다 |
...
| ② | `npx tsc --noEmit`, `npm run lint`, `./gradlew test` (metadata 는 여기 없다. 묶음 ③ 으로 옮겼다) |
| ③ | `npm run check`(`wallet` 18 + `plans` 4, 수정 전 각각 4건·2건 빨간불), `npx tsc --noEmit`, `npm run lint`, 브라우저 실측: 카드 2장 등록 시 두 번째가 서랍 밖에 보이는가, 기본 카드를 DB 에서 없앴을 때 목록이 보이는가, `/api/plan` 을 죽였을 때 안내가 나오는가 |
...
- C-1 의 브라우저 항해는 **로컬 dev 서버에서** 확인했다. 운영(Vercel)에서 직접 누르지는 않았다.
- C-1 재현에 **가짜 토큰**을 썼다. `/auth` 의 로그인 가드가 토큰의 유효성을 보지 않고 존재만 보기 때문에 성립한다. 진짜 로그인 직후 경로는 같은 `nextPath()` 를 쓰므로 같은 결과지만 따로 누르지 않았다.
...
- **B-4 와 접근성·metadata 는 자동 검사가 없다.** 브라우저와 눈으로만 지켜진다. 프론트 테스트 러너가 없어서이고, 들이는 것은 별도 슬라이스다.
