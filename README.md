...
⚠️ 간접 증거가 하나 있다. 문서 충돌 스캔 검증에 "실제 PDF 51쌍 0 오탐" 이라는 기록이 있고
(`docs/decisions.md` 2026-08-05 항목), 그 쌍이 만들어지려면 PDF 가 청킹·임베딩까지는 갔어야 한다.
다만 그 실행에서 `ready` 로 바뀌는 것을 직접 확인한 기록은 아니고, 쓴 파일도 남아 있지 않다.
