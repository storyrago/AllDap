...
실제 한글 PDF·DOCX 파싱은 W1 때 실제 파일로 확인했고(`docs/W1-이해노트.md`의 검증 표),
이후 문서 충돌 스캔을 실제 PDF 쌍으로 돌린 기록도 있다(`docs/decisions.md`).
**대용량 문서는 여전히 안전하다고 말할 수 없다.** pymupdf가 PDF를 통째로 메모리에 올려서
운영 인스턴스(1GB)에서는 OOM 방아쇠가 된다. 상한을 환경변수로 빼둔 이유가 그것이다(`docs/DEPLOY.md`).
