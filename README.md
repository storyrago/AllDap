...
- ✅ **파서 단위**(파일 → 텍스트 추출)는 실제 한글 파일로 확인됐다.
  `docs/W1-이해노트.md` 의 검증 표: PDF "2페이지 정상 추출" · DOCX "본문 + 표까지".
- ❌ **업로드 파이프라인 종단**(업로드 → 청킹 → 임베딩 → `ready`)을 PDF·DOCX 로 돌린 기록은 없다.
  W1 종단 실측은 전부 텍스트 코퍼스 기준이다.
- ❌ **대용량 문서**도 미검증이다. pymupdf 가 PDF 를 통째로 메모리에 올려서 운영 인스턴스(1GB)
  에서는 OOM 방아쇠가 된다. 업로드 상한을 환경변수로 빼둔 이유가 그것이다(`docs/DEPLOY.md`).

🔴 **그리고 지금은 그 ✅ 를 다시 재현할 수 없다. 저장소에 PDF·DOCX 파일이 하나도 없다**
(`ai-service/testdata/corpus/` 는 50개 전부 `.md`). 그때 쓴 파일이 남아 있지 않아서,
**파서를 손대는 순간 위 ✅ 는 근거가 되지 못한다.** 파서를 고칠 일이 생기면
검증용 샘플 파일을 저장소에 넣는 것이 먼저다.

⚠️ 간접 증거가 하나 있는데 **PDF 에만 해당한다.** 문서 충돌 스캔 검증에 "실제 PDF 51쌍
0 오탐" 이라는 기록이 있고(`docs/decisions.md` 2026-08-05 항목), 그 "쌍" 은 청크 쌍을
pgvector 최근접으로 고른 것이라 **PDF 가 청킹·임베딩까지는 갔다는 뜻**이다.
그런데도 위에서 ❌ 로 둔 이유는 셋이다.
① 그 기록이 파일 출처를 안 적었다("실제 PDF" 가 파일 형식을 말하는지가 확실하지 않다)
② `status=ready` 로 바뀌는 것을 본 기록이 없다
③ Spring 을 거쳐 올렸다는 기록도 없다.

🔴 **그리고 DOCX 는 그 간접 근거조차 없다.** PDF 와 DOCX 를 한 묶음으로 말하지 말 것.

**"안 나빠졌다" 와 "재보지 않았다" 는 다르다.** 여기는 후자다.
