...
public interface EvalRunRepository extends JpaRepository<EvalRun, UUID> {
    ...
    List<EvalRun> findAllByBotIdOrderByCreatedAtDesc(UUID botId);

    /** 봇 카드에 보여줄 "가장 최근 평가 점수" 용. */
    Optional<EvalRun> findFirstByBotIdOrderByCreatedAtDesc(UUID botId);

    Optional<EvalRun> findByIdAndBotId(UUID id, UUID botId);
}
