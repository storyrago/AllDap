...
public class AiServiceCircuitBreaker {
    ...
    private volatile long openedAtNanos = CLOSED;
    ...
    public boolean allowRequest() {
        ...
        if (elapsed < properties.circuitOpenDuration().toNanos()) {
            return false;   // OPEN — 아직 쉬는 중
        }
        return true;        // HALF_OPEN — 살아났는지 한 번 두드려 본다
    }

    /** 호출이 성공했다. 서킷을 닫고 카운터를 리셋한다. */
    public void recordSuccess() {
        if (openedAtNanos != CLOSED) {
            log.info("[AI 서킷] 닫힘 — Python 이 응답했다.");
        }
        consecutiveFailures.set(0);
        openedAtNanos = CLOSED;
    }
