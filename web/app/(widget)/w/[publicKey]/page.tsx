...
function Assistant({
  ...
  return (
    <div className="max-w-[90%]">
      ...
      {/*
       * 🔴 여기는 <파일명만> 그린다. 관리자 채팅(app/(dashboard)/bot/[botId]/chat)과 일부러 다르다.
       *
       * 응답의 Source 에는 score(0~1)와 preview(앞 200자)가 <이미 들어 있다.> 안 오는 게 아니라
       * 안 쓰는 것이다. 못 만든 것과 안 만든 것을 구분하려고 이 주석을 남긴다 (2026-09-16).
       *
       * 왜 점수를 안 보여주나 — 보는 사람이 다르기 때문이다.
       *   관리자: 봇을 고치는 사람이다. "57%" 가 컷오프를 조일지 말지의 판단 재료가 된다.
       *   방문자: 고칠 수 없는 사람이다. 57% 를 해석할 기준이 없고, 낮은 숫자가 붙으면
       *           <맞는 답까지> 못 믿게 된다. 관리자 채팅 쪽 같은 자리 주석의 우려
       *           ("찾았는데 왜 답을 안 해?" 로 읽힌다)가 방문자에게는 더 크게 걸린다.
       *
       * 🔴 <원문 미리보기(preview)는 이야기가 다르다.> 숫자와 달리 방문자도 읽고 판단할 수 있어
       * 오히려 설득력이 있다. 2026-09-16 에 "파일명을 눌러 근거 문장을 펼친다" 를 검토했고
       * 지금은 <안 한다>로 정했다. 기각 사유는 "필요 없다" 가 아니라 <별도 슬라이스>다 —
       * 좁은 iframe 안의 펼침 UI 라 모바일 폭까지 같이 봐야 한다(위젯은 모바일이 범위 안이다).
       * 만들 때는 관리자 쪽 Sources 컴포넌트의 splitSource 를 재사용할 것. 제목 줄(## …)을
       * 떼어내는 규칙이 거기 있고, 두 곳이 갈리면 같은 근거가 다르게 보인다.
       */}
