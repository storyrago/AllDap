...
/* id 는 BIGINT 순번이라 숫자다 (2026-09-13 전환). 전에는 "a"·"b" 같은 문자열이었다. */
function card(id: Id, isDefault: boolean): BillingCard {
  return {
    id,
    issuerCode: "61",
    issuerName: "현대",
...
}
{
  ...
  check("1장: 그 카드가 청구 카드다", v.billed?.id, 1);
...
{
  const [a, b] = [card(1, true), card(2, false)];
  const v = walletView([a, b], null);
  check("2장: 청구 카드는 서랍에 없다", v.others.map((c) => c.id), [2]);
  check("2장: 서랍은 닫혀 있다", v.drawerOpen, false);
}
{
  const v = walletView([card(1, true), card(2, false), card(3, false), card(4, false), card(5, false)], null);
...
{
  const [a, b] = [card(1, false), card(2, false)];
  const v = walletView([a, b], null);
  check("기본 없음: 서랍을 그린다", v.showDrawer, true);
  check("기본 없음: 두 장 다 서랍에 있다", v.others.map((c) => c.id), [1, 2]);
...
}
{
  ...
  const v = walletView([card(1, false)], null);
  ...
  check("기본 없음 1장: 그 카드가 서랍에 있다", v.others.map((c) => c.id), [1]);
}
{
  const v = walletView([card(1, false), card(2, false), card(3, false), card(4, false), card(5, false)], null);
...
{
  const v = walletView([card(1, true), card(2, false)], 2);
  check("방금 추가: 서랍을 열어둔다", v.drawerOpen, true);
}
{
  const v = walletView([card(1, true)], 1);
