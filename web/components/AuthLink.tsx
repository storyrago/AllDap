 ...
 *
 * ── 🔴 `if (token)` 이 <기한까지> 본다 (2026-09-09) ────────────────────────
 * 이 줄은 그대로인데 뜻이 달라졌다. `getAccessToken` 이 만료된 토큰을 null 로 걸러주기
 * 때문이다. 그전에는 <존재만> 봐서, 어제 로그인하고 오늘 온 사람에게 "대시보드" 를 띄우고
 * 눌러 들어가면 401 안내를 보게 했다. 버튼이 <거짓말>을 한 것이다.
 * 판정을 여기가 아니라 `getAccessToken` 안에 둔 이유는 그 함수 주석에 적었다:
 * 같은 질문을 하는 자리가 다섯 곳이라, 여기서만 고치면 화면마다 로그인 상태가 갈린다.
 *
