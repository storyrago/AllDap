 ...
 *   TODO(W2): `web/app/w/[publicKey]/page.tsx` 를 만들 것. 아직 `web/` 자체가 없다.
 *
 * ===========================================================================
 ...
 * [현재 상태 — 거짓 완성 금지]
 * ===========================================================================
 *   ★ 지금은 플로팅 버튼·패널 열고 닫기까지만 실제로 동작한다. ★
 *   · GET /api/w/{publicKey}/config  → Spring(:8080)이 아직 없어 실패한다 (W2)
 *   · iframe 안의 /w/{publicKey}     → Next.js(:3000)가 아직 없어 로드되지 않는다 (W2)
 *   실패해도 콘솔 에러로 죽지 않고, 패널 안에 한국어 안내 문구를 띄우도록 만들어 뒀다.
 *
 *   TODO(W2): Spring에 GET /api/w/{publicKey}/config 구현 + CORS 허용
 *             (설치된 남의 도메인에서 호출되므로 CORS 없이는 무조건 막힌다)
 *   TODO(W2): Next.js에 /w/[publicKey] 페이지 구현 (채팅·출처·피드백 전부 여기)
 *   TODO(W2): allowed_origins 검증 — PRD 부록A #3은 "필드만 만들고 검증은 이후"가 현재 기울기.
 *             검증을 켜기 전까지는 public_key만 알면 누구나 남의 봇을 붙일 수 있다는 뜻이다.
 *   TODO(W2): rate limit — 익명 공개 엔드포인트라 그대로 두면 LLM 비용이 샌다.
 * ---------------------------------------------------------------------------
 */
(function () {
  'use strict';
   ...
   * 리버스 프록시로 한 도메인에 둘을 붙이면(예: /api/* 와 /widget/* → Spring,
   * 나머지 → Next.js) origin 이 같아져 아래 폴백(스크립트 origin)이 그대로 맞는다.
   *   TODO(W2): 위 프록시 구성을 정하고, 정해지면 이 주석과 README 를 갱신할 것.
   */
  ...
  /* config 응답이 오기 전까지 쓸 기본값.
     TODO(W2): bots 테이블에 색상 컬럼이 아직 없다(db/V1__init.sql 확인).
  ...
  function getJson(path) {
    ...
    return fetch(API_BASE + path, options).then(function (res) {
      ...
      // TODO(W2): Spring 이 뜨기 전까지는 항상 이 경로를 탄다.
